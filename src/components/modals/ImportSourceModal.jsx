...
function ImportSourceModal({ onFromFile, onFromUrl, onFromGist }) {
  ...
  useEffect(() => {
    getGistService().then((svc) => {
      setHasGistService(!!svc)
      setHasGistId(!!svc?.metadata?.gistId)
    })
  }, [])
  ...
  function handleFromGistWithUrl() {
    const trimmed = url.trim()
    if (!trimmed) return
    closeModal()
    onFromGist?.(pw, trimmed)
  }
  ...
  const gistValid = !!extractGistId(url)
    ...
    return (
      <div className="flex flex-col min-h-0 flex-1">
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-4 space-y-4">
          <div>
            <label className="block text-sm font-medium text-text mb-2">
              {t('database.importModal.gistUrlLabel')}
            </label>
            <input
              type="url"
              value={url}
              onChange={(e) => setUrl(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && gistValid) handleFromGistWithUrl()
              }}
              placeholder={t('database.importModal.gistUrlPlaceholder')}
              className="w-full min-h-[44px] px-3 py-2 rounded-lg border border-border bg-surface text-text text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
              autoFocus
            />
            {url && !gistValid && (
              <p className="text-xs text-error mt-1.5">
                {t('database.importModal.gistUrlInvalid')}
              </p>
            )}
            <p className="text-xs text-tertiary mt-1">{t('database.importModal.gistUrlDesc')}</p>
          </div>
          <form onSubmit={(e) => e.preventDefault()}>
            <input
              type="text"
        ...
        <div className="flex justify-end gap-3 px-6 py-4 shadow-section shrink-0">
          ...
          <button
            type="button"
            onClick={handleFromGistWithUrl}
            disabled={!gistValid}
            className="min-h-[44px] px-4 btn-primary text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {t('database.importFromGist')}
          </button>
        </div>
      </div>
    )
